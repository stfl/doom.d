...
:PROPERTIES:
:ID:       a7b6af80-732c-4999-ac31-d328bf5788cd
:END:

=auto-save-default= is Emacs's separate crash-recovery mechanism: it
periodically writes =#file#= snapshots without touching the visited
file itself.

=auto-save-visited-mode= saves every visited buffer to its actual file
once it has been idle for =auto-save-visited-interval= seconds. That
interval is set to 30 seconds rather than the 5-second default,
because =before-save-hook= does expensive work on some files (org
encryption, formatters), so a longer interval avoids re-running that
on every short pause.
...
** Calendar
:PROPERTIES:
:ID:       687712a4-c515-4910-9ee1-d4f0bb994678
:END:

Start the week on Monday, matching the =de_AT= locale; Emacs defaults to Sunday.
This governs how the calendar is drawn — =M-x calendar=, the =org-read-date=
popup, and anything else built on the calendar grid.

Org's clock ranges are independent of this variable: =org-clock-special-range=
never reads =calendar-week-start-day=, and takes its week start from the
=:wstart= parameter, which =org-clocktable-defaults= already sets to =1=
(Monday). So clocktable and clockmatrix weekly reports are unaffected either way.
...
:PROPERTIES:
:ID:       d199ad7a-c9b4-4452-9eca-fe45320851a5
:END:
...
:PROPERTIES:
:ID:       1a52abb7-a016-4ca4-bb0f-7fdc2cdf4fd3
:END:
...
** Garbage Collection
:PROPERTIES:
:ID:       50de3043-f7dc-4e82-bd21-3c931f64aa9a
:END:
...
:PROPERTIES:
:ID:       e5fd33fe-83ec-4c3b-8e6c-a1f5236c3b9e
:END:
...
:PROPERTIES:
:ID:       2ba1d830-2515-4104-b3b0-8e024da10150
:END:
...
** tab-width
:PROPERTIES:
:ID:       ae3f2bb8-24b3-4326-baf1-c6b13e9d97bf
:END:
...
Rebuild the ID index once Emacs has been idle for 25 seconds after start. The
index is built from configuration alone: org-roam files, agenda files and
=org-id-extra-files=. Files that held IDs in an earlier session only because
they were open are not carried over, so the index after a restart depends only
on configuration.
...
Storing a link to a heading uses its :ID: when the file is indexed above, and
a :CUSTOM_ID: slug otherwise, so links out of files org-id never scans still
resolve.
...
** agile-gtd
:PROPERTIES:
:ID:       b82acc8f-f3ba-448b-be37-cf5990b7727e
:END:
...
** org-mcp
:PROPERTIES:
:ID:       1523bb4e-9612-452f-b7fa-834f09dd4a89
:END:
...
Doom rebinds =user-emacs-directory= to =~/.config/emacs/.local/cache/=,
so =mcp-server-lib-install-directory= (which defaults to
=user-emacs-directory=) lands the stdio shim under =.local/cache/= rather
than alongside the rest of the Doom config. Pin it to =~/.config/emacs/bin/=
so the installed script matches upstream's documented layout.
...
A view is a named question, a filter a named restriction, and a range how
much. Composing the three reaches every agenda this workflow has out of two
small lists, so =org-view= answers what the agenda commands answer. A filter is
always a *name*: a caller handed an expression slot invents predicates that do
not exist, and =org-query= is there for one that genuinely wants to write a
query.

=org-mcp-filters= is left unset. The filters belong to the workflow rather than
to this file: one per project tag, built the way agile-gtd builds the tag
filter of that project's own agenda command, so a filter narrows a view
exactly as the agenda narrows. That makes it something =agile-gtd= sets as it
loads, beside the query builders the views already name, and it lands with the
workflow rework.

Until then the four views run unrestricted, and naming a filter is refused with
an empty list of valid names rather than a wrong one.

#+begin_src elisp
(use-package org-mcp
  :after (org agile-gtd)
  :custom
  ;; Values this workflow works out as a node is read.  They arrive under
  ;; `computed', apart from the node's own fields and from its Org drawer,
  ;; so a client cannot write one back as though the file held it.
  (org-mcp-computed-fields '((parent-priority . agile-gtd--direct-parent-priority)
                             (rank . agile-gtd--item-rank)))
  ;; Each view is called with the parameters it declares and no others, in
  ;; the order filter then range, which is exactly the argument order the
  ;; agile-gtd query builders take.  The first range listed is the one the
  ;; view runs at unasked.
  (org-mcp-views
   '((inbox   :name "Inbox"
              :query agile-gtd-agenda-query-inbox)
     (stuck   :name "Stuck Projects"
              :query agile-gtd-agenda-query-stuck-projects
              :filter t)
     (next    :name "Next Actions"
              :query agile-gtd-agenda-query-next-actions
              :filter t
              :range (sprint backlog all someday))
     (backlog :name "Backlog"
              :query agile-gtd-agenda-query-backlog
              :filter t
              :range (all sprint backlog someday))))
...
;; nil lets org-mcp resolve `org-agenda-files' on every call, so a file added
;; during the session — by agile-gtd or by `org-agenda-file-to-front' — is in
;; scope at once.
(setopt org-mcp-allowed-files nil)

;; t lets a call reach an Org file outside the allowed ones by naming its
;; path, the decision-loop DECISIONS.org files among them.  The permission
;; covers writing as well as reading, and lasts only for the call that names
;; the file: an ID, or the file of the running clock, still resolves within
;; the allowed files.
(setopt org-mcp-file-scope-override t)
#+end_src

** Appearance
:PROPERTIES:
:ID:       901fb4ac-ff01-4b60-8c3f-0765145039ba
:END:
...
** Tag colors
:PROPERTIES:
:ID:       04f7619c-a558-45c0-9e30-99e476a8304a
:END:
...
** org-checklist
:PROPERTIES:
:ID:       2c3b9bf8-b761-45cc-8a68-c8dc3bf83407
:END:
...
** org-habit
:PROPERTIES:
:ID:       a3b7de50-1a49-47fe-b49d-7f580fd9ac7a
:END:
...
** org-clock
:PROPERTIES:
:ID:       29144cf1-c672-4f69-98a2-e7dead92ad04
:END:
...
Clocking in and out at a time read from the minibuffer. Continuation from the
last clock-out is =org-clock-projects='s, scoped to a project.
...
** org-clock-projects
:PROPERTIES:
:ID:       7afb18e0-0f6c-48dd-9e63-b5aae3533e86
:END:

Per-project parallel Org clocks: each project file owns at most one open clock
and its own last clock-out time, so time spent bouncing between engagements is
recorded as it happens and continuation never bridges from one client to
another. Dormant until a project is created.

The same package reports on what those clocks recorded: =org-clock-projects-export=
writes one project's entries for a chosen period to CSV,
=org-clock-projects-check= names everything that would make such an export
untrustworthy, and the =clockmatrix= dynamic block renders clocked time as
periods against projects. All three resolve a period through one core, so a
matrix cell and a CSV describe the same stretch of time.
...
With parallel clocks, dangling clock lines are the normal state. Org's
automatic resolution otherwise prompts about every other project on each
clock-in made while no clock is running.
...
The recipe above tracks GitHub without a =:pin=, so every =doom sync -u= can
change the package underneath this configuration. What breaks then is invisible
from here: advice that no longer attaches to =org-clock-in=, a handler no longer
swapped into =kill-emacs-query-functions=, a resolver record that stops carrying
=:tag=, =:name= and =:file= separately. None of it raises an error. The clocks
simply stop behaving the way this section describes.

=test/org-clock-projects-smoke.el= asserts those seams, together with the CSV
contract configured under "org-clock-csv row format" below:
...
*Run it after any =doom sync -u= that moves this package, and change it in the
same commit as the configuration it describes.* It holds the seam between the
two, so a package release that renames =org-clock-projects--clock-in-advice= or
reshapes a project record has to land beside the assertion that names it.

The test asserts nothing this file already sets. A check that goes red only
because a setting here changed on purpose catches no bug and is repaired by
copying the new value across, so values, paths and keybindings stay out of it;
keymap coverage lives in =test/agile-gtd-keys.el=.

** org-clock-csv row format
:PROPERTIES:
:ID:       c43b236b-8e46-4bfe-93a8-54fa8054c060
:END:

The columns an exported CSV carries. =org-clock-projects-export= heads the file
with =org-clock-csv-header= and writes each row through
=org-clock-csv-row-fmt=, so the shape of the file is settled here while the
exporter decides only which entries reach it. Beyond the stock columns the row
carries what the invoice build reads: the =ARCHIVE_OLPATH= and =ARCHIVE_ITAGS=
Org writes when a subtree is archived, and the =AP= and =TICKET= properties.
...
:PROPERTIES:
:ID:       97a5ea17-5a69-4553-80ac-2ea7a227e4fe
:END:
...
** Org Pandoc Import
:PROPERTIES:
:ID:       7ce596c3-39b2-47b6-914c-1a4152d46e50
:END:
...
:PROPERTIES:
:ID:       8321fb56-4978-4833-bcbe-d740fbd971ee
:END:
...
:PROPERTIES:
:ID:       09296f88-0c61-4a88-afd4-a9a7109cc402
:END:
...
orgzly-formatter already removes trailing whitespaces and keeps certain ones in
specific cases.
...
** ox-hugo
:PROPERTIES:
:ID:       0cca10e7-aec0-4e0b-8711-672dadb943a7
:END:
...
** ox-zola
:PROPERTIES:
:ID:       f7100dd9-c142-4e29-9a45-5523f8b04512
:END:
...
** Key bindings
:PROPERTIES:
:ID:       d2e72fb9-7e0e-4eb5-9572-baa52d992809
:END:

Three chord families act on the agenda line under point and on the view around
it. Both vertical pairs run along the priority axis, and up means toward =A= in
each: a higher priority for the item, a narrower range for the view. Horizontal
is the TODO state.

| Chord           | Acts on        | Effect                             |
|-----------------+----------------+------------------------------------|
| ~C-S-k~ ~C-S-j~ | the item       | priority up / down                 |
| ~C-M-k~ ~C-M-j~ | the whole view | narrower / wider ~agile-gtd~ range |
| ~C-S-h~ ~C-S-l~ | the item       | TODO state back / forward          |

~C-S-h~ and ~C-S-l~ take the keys ~evil-org-agenda~ gives
~org-agenda-todo-previousset~ and ~-nextset~. Those step between TODO keyword
/sequences/, and this configuration declares exactly one, so they have nothing
to step to. Stepping within the sequence is what an Org buffer's ~H~ and ~L~ do,
and delegating to ~org-agenda-todo~ with a directional argument reproduces that
exactly, including where it stops at the ends.
...
The view range is ~agile-gtd~'s: ~sprint~, ~backlog~, ~all~ and ~someday~,
narrowest first. Each agenda command opens at the range it declares, and
rotating is local to the open agenda, so widening one view leaves every other
one alone and quitting discards the widening.

~agile-gtd-agenda-reset-show-priorities~ keeps a name but loses its key; it puts
an agenda back to its declared range and is reachable with ~M-x~.
...
** Intermediate checkbox state
:PROPERTIES:
:ID:       b6ab1892-a09d-4320-8cdf-20e1b70b3705
:END:

Doom binds ~SPC m x~ to ~org-toggle-checkbox~, which flips =[ ]= ↔ =[X]=. ~SPC m X~
sits next to it and sets the intermediate =[-]=. Plain ~org-toggle-checkbox~
only reaches that state through a double prefix argument, which is awkward
under evil where ~C-u~ scrolls.
...
:PROPERTIES:
:ID:       0448573b-0543-451d-b006-cec1b874389c
:END:
...
:PROPERTIES:
:ID:       91fbc4e4-614e-4c18-ae12-95251ccdf295
:END:
...
** age.el — read agenix-encrypted files
:PROPERTIES:
:ID:       f2a884c9-3ec5-4e37-a6f7-29782c148c4e
:END:

Transparently decrypt/encrypt =.age= files (agenix secrets under
=~/.config/dotfiles/secrets/=) when visiting them, analogous to =epa-file= for GPG.

Identity is the same ed25519 SSH key used by agenix ([[file:~/.config/dotfiles/secrets/secrets.nix][secrets.nix]]).
...
The document look is settled in pandoc's user data directory, not here.
=~/.local/share/pandoc/defaults/pdf.yaml= names the whole PDF export — Typst as
the engine, A4, the body font, a column-width filter and a Typst style include —
and =reference.docx= beside it does the same for Word. All of it is installed by
=modules/home/pandoc.nix= in the dotfiles flake, so the same export works from a
shell:

#+BEGIN_SRC sh :tangle no
pandoc report.org -o report.pdf --defaults=pdf
#+END_SRC

ox-pandoc stats every file-valued option before it runs, so =defaults= has to be
spelled as a path here rather than as the bare name pandoc would resolve itself.

A document keeps control of what is its own, through its own keywords:

| =#+LANGUAGE: de=          | hyphenation, and the heading /Inhaltsverzeichnis/     |
| =#+OPTIONS: ^:nil=        | stops Org reading =a_b= as a subscript                |
| =#+OPTIONS: email:t=      | carries =#+EMAIL:= into the source pandoc is handed   |
| =#+EMAIL:=                | the address, on its own line under the author       |
| =#+PANDOC_OPTIONS: toc:t= | asks for a table of contents                        |

Org drops =#+EMAIL:= from the source it generates for pandoc unless
=email:t= is set, so the address needs both keywords to appear.

Export with =C-c C-e p t= for PDF, =C-c C-e p d= for Word.
...
** Typst
:PROPERTIES:
:ID:       6a243e55-0025-47a3-a084-f44780050e1a
:END:
...
** pdf-tools
:PROPERTIES:
:ID:       e803cc94-94f5-44db-acff-dd2f7e30f3bc
:END:
...
:PROPERTIES:
:ID:       5933613a-bb9c-47c3-8413-5808a0b007f2
:END:
...
** ssh-deploy
:PROPERTIES:
:ID:       a6bc4c2c-7141-4d64-944b-3e0661ba2dc1
:END:
...
** zoxide
:PROPERTIES:
:ID:       ae6f3ad6-e503-4f5b-80ab-cb308eabe56d
:END:
...
Doom's =:completion vertico= module already binds =consult-dir= to =C-x C-d=,
and to =C-x C-j= inside the minibuffer, but ships no zoxide source. This adds
one, reachable by narrowing with =z=. The =:category 'file= is what earns
marginalia annotations and embark actions on the candidates; =zoxide.el='s own
=zoxide-open-with= omits it, which is why this is preferred over that package.
...
* Coding
:PROPERTIES:
:ID:       bc8bb4b1-bd36-47cb-9e57-b2cad1c8ae6a
:END:
...
** ielm REPL
:PROPERTIES:
:ID:       9fba0fb4-8826-4db4-89b8-66fc265f477c
:END:
...
** lsp-mode
:PROPERTIES:
:ID:       7acba454-67fe-4109-9c4b-7d2ef4001020
:END:
...
*** Key Mappings
:PROPERTIES:
:ID:       bcce84aa-9ee4-41e0-89ae-3f64b93d099b
:END:
...
*** Options
:PROPERTIES:
:ID:       92a1e07d-27f2-493b-baf0-fe44e6b6a874
:END:
...
*** emacs-lsp-booster
:PROPERTIES:
:ID:       62ad3196-eb07-4d88-bc4b-3187b1684df7
:END:
...
** Flyover
:PROPERTIES:
:ID:       62d522b2-0bdb-4ca1-a04b-fd20d5a63874
:END:
...
** Code Formatting
:PROPERTIES:
:ID:       74b872e2-13dd-432f-870f-be4e36a0f3ba
:END:
...
** Python
:PROPERTIES:
:ID:       3a459807-776b-4969-8e8d-366b2b3fe8a1
:END:
...
*** poetry
:PROPERTIES:
:ID:       28809999-fe05-492d-9fad-0f11d22ed045
:END:
...
*** conda
:PROPERTIES:
:ID:       acf4f133-1f92-4ce4-865c-cbb25f2058f8
:END:
...
*** numpydoc.el
:PROPERTIES:
:ID:       fcd296a2-e589-4ff5-815d-4484119f1538
:END:
...
:PROPERTIES:
:ID:       ff74a51f-2141-4800-b316-6bb7681d4739
:END:
...
*** emacs-jupyter
:PROPERTIES:
:ID:       fa329006-bdaa-44c3-bc67-ce78ce627a70
:END:
...
:PROPERTIES:
:ID:       d1bed36f-3297-4f46-945f-b46343c72c33
:END:
...
** Rust
:PROPERTIES:
:ID:       1daf5203-a457-4bae-a1e7-0d5f2d237582
:END:
...
** Nix
:PROPERTIES:
:ID:       71ef306f-1cef-46c7-9556-f6745b06cac4
:END:
...
:PROPERTIES:
:ID:       c99ebabb-3530-4c9c-8181-54eadb8ff64a
:END:
...
:PROPERTIES:
:ID:       238198c7-685d-4b4d-8f04-daf1ff0ff958
:END:
...
** JSON
:PROPERTIES:
:ID:       db2bae45-fcb9-484f-acb3-0f851db125e5
:END:

The =:lang json= module handles =.json= → =json-ts-mode= via tree-sitter.
...
** Perl
:PROPERTIES:
:ID:       94cafd40-5143-4ef0-bbce-30a204374409
:END:
...
** Logview
:PROPERTIES:
:ID:       3616b25e-953f-4156-ae59-1c94d6995560
:END:
...
*** LTEX
:PROPERTIES:
:ID:       81fcbed1-64ea-4d05-9ebb-06f401bed754
:END:
...
*** ispell
:PROPERTIES:
:ID:       676bb9b3-b404-47ab-8857-10cd52f67f68
:END:
...
*** AsciiDoc (adoc)
:PROPERTIES:
:ID:       c8a49076-4191-41a5-9b9c-70198386dc77
:END:
...
** ssh-config-mode
:PROPERTIES:
:ID:       6cf36285-c9d8-4e84-92f1-fb85c013e5fa
:END:
...
** BitBake
:PROPERTIES:
:ID:       96efe644-1aa3-437c-9756-fdc2aaef7ead
:END:
...
** Meson build system
:PROPERTIES:
:ID:       be3dbfd3-334e-4b59-aea5-69d16d356146
:END:
...
** C/C++
:PROPERTIES:
:ID:       ee43f254-70ea-43cb-a736-0c52cd29a890
:END:
...
** Turbo Log
:PROPERTIES:
:ID:       78fbe186-31b7-4974-bfa8-322a08204ea8
:END:
...
** just
:PROPERTIES:
:ID:       29eab315-d708-42fe-9e95-c3a3a28d26c7
:END:
...
** Magit
:PROPERTIES:
:ID:       ca5c16c5-fadc-4c44-b4b2-9b286deae2ba
:END:
...
:PROPERTIES:
:ID:       f5268c2a-667d-4db6-8131-ae71f0db1c7f
:END:
...
*** forge-azure
:PROPERTIES:
:ID:       04ac7ce6-9e67-4860-a793-c8ab47ac5762
:END:

Azure DevOps backend for forge, so ÖBB pull requests are reviewable from magit.
...
Authenticates with the PAT in password-store at =OEBB/dev.azure.com= — the same
entry =~/work/oebb/.envrc= feeds to the az CLI. auth-source-pass matches an
entry by its trailing path segment, so the leaf name is what binds it to host
=dev.azure.com=; the directory above it is free. Entra ID auth is not an
option: there is no ~az login~ session, only the PAT. The matching username
comes from =azure.user= in git config, set globally from =oebb.nix=.
...
*** magit-todos
:PROPERTIES:
:ID:       b93de293-9ed1-43e9-8d6b-435433c1423b
:END:
...
*** mailscripts.el
:PROPERTIES:
:ID:       74b33175-e7d9-4cfa-a40d-898f4b332a85
:END:
...
** diffview
:PROPERTIES:
:ID:       6b680599-8922-4de7-9aff-3607e847c731
:END:
...
** blamer.el
:PROPERTIES:
:ID:       af3f1e2d-4f51-43a3-948e-ffc014645d82
:END:
...
** beads.el
:PROPERTIES:
:ID:       c95f10fe-5d12-44ac-9d2b-2304694f27bf
:END:

[[https://github.com/r0man/beads.el][r0man/beads.el]] — an Emacs interface to the
[[https://github.com/steveklabnik/beads][Beads]] git-backed issue tracker (the
~bd~ CLI), which keeps issues in an embedded Dolt database under =.beads/=.
It is a Dolt-native client: Magit-style transient menus (=M-x beads=), an
issue dashboard, eldoc, and dedicated =bd dolt= server/commit/push
commands.  It drives ~bd~ directly and finds the project by walking up to
a =.beads/= directory, so it works against the Dolt store out of the box.

ctietze/beads.el (its Dolt-fixed =upgrade= branch on Codeberg) is an
alternative front-end, but it defines the same ~beads~ feature and
commands, so only one can be installed at a time.

~bd~ reads the embedded Dolt store under =.beads/= with no server running.
~stfl/beads-dolt-port~ is the fallback for when that auto-discovery does
not work and ~bd~ must instead reach a running Dolt SQL server: set it to
that server's port and beads.el points ~bd~ at it.
...
* AI
:PROPERTIES:
:ID:       2e541c46-3fc4-4e52-bd1c-3d032ff5249a
:END:
...
** gptel
:PROPERTIES:
:ID:       8a59e4db-5f9c-45ec-aa0e-bb8141796553
:END:
...
** gptel-magit
:PROPERTIES:
:ID:       db208ba3-eca2-4511-bdbd-355f37f0bceb
:END:
