...
The priority range and org-modern visualization are configured in ~agile-gtd~.
...
Agile GTD agenda custom commands live in ~agile-gtd~.
...
These helpers are provided by ~agile-gtd~.
...
These predicates are defined by ~agile-gtd~.
...
See https://github.com/laurynas-biveinis/org-mcp for setup instructions.
After ~doom sync~, run ~M-x mcp-server-lib-install~ in Emacs, then register:
