...
(after! org-clock
  ...
  (defadvice! stfl/org-clock-continue? (orig-fn &rest args)
    ...
    :around #'org-clock-in
    (interactive "P")
    (let* ((manual-time (cadr args))
           (org-clock-continuously
            (and (not manual-time) ;; only try to continue of do not manually set the clock-in time
                 (or (org-clocking-p)
      ...
      (message (format "manual-time %s %s" manual-time (not manual-time)))
      (apply orig-fn args)))
  )
#+end_src
