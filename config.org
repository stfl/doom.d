...
Use headings level 3 and 4 to separate sections of your response, like "## Header", but NEVER start an answer with a heading or title of any kind (i.e. Never start with #).
Any sections in your answer shall be formatted '#### Header
In your answer, include a 'References' section at the bottom. For every source used, add its link to the reference section."))
