...
Invalid indentation style is a common issue at my work.
...
Visually highlight if an indentation issue was discovered which emacs already does for us
...
(after! notmuch
  (setq +notmuch-sync-backend 'mbsync
        ...
        '((:key "i" :name "inbox"   :query "tag:inbox and not tag:archive")
          ...
          (:key "m" :name "my PRs (new)"  :query "tag:my-pr and not tag:archive and not tag:killed and not tag:deleted")
          (:key "w" :name "watch (new)"   :query "tag:watch and not tag:archive and not tag:killed and not tag:deleted")
          (:key "t" :name "team"    :query "tag:lists/team and not tag:archive")
