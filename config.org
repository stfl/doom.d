...
How a table looks in an exported document is settled in pandoc's user data
directory, not here: =~/.local/share/pandoc= holds a column-width Lua filter,
a Typst header include and a docx reference document, all installed by
=modules/home/pandoc.nix= in the dotfiles flake. Pandoc finds =reference.docx=
on its own; the options below are what points ox-pandoc at the other two.

Typst is the PDF engine, so no TeX distribution is involved. Its template
needs a =mainfont= — without one pandoc fails with /font fallback list must
not be empty/.

Each document exported this way wants =#+OPTIONS: ^:nil= in its header. Org
reads =cpu_temperature_sensor= as =cpu= with a subscript, which mangles every
