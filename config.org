...
:END:
:LOGBOOK:
CLOCK: [2025-10-14 Di 07:30]--[2025-10-14 Di 13:30] =>  6:00
:END:
...
** org-clock
:LOGBOOK:
CLOCK: [2025-10-14 Di 13:00]--[2025-10-14 Di 14:00] =>  1:00
:END:
...
(after! org-clock
  ...
  (defadvice! stfl/org-clock-continue? (orig-fn &rest args)
    ...
    (let* ((start-time (cadr args))
           (continue (and (not start-time) ;; only try to continue of do not manually set the start-time time
                          (or (org-clocking-p)
           ...
           ;; (org-clock-continuously (and continue )
            ))
      (apply orig-fn args)))
  )
#+end_src
