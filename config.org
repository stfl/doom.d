...
Allow writing a file or an entire project to a remote location after local save.

#+begin_src elisp
(defvar stfl/ssh-deploy-local-mappings
  ;; AI create a global alist to store local to remote mappings (string . string)
  )

(defun stfl/ssh-deploy-clear-mappings ()
  (interactive)
  ;; AI for each mapping in stfl/ssh-deploy-local-mappings, remove the buffer local variables and remove from the alist
...
(defun stfl/ssh-deploy-register-local-mapping (&optional remote-path)
  (interactive (if current-prefix-arg
                 ...
                 (list (read-file-name "Remote path: " nil nil nil
  ...
  (if current-prefix-arg
      (progn
        ...
        ;; AI add (local . remote) to stfl/ssh-deploy-local-mappings
        )
    (progn
      (setq-local ssh-deploy-root-local (expand-file-name (buffer-file-name))
                  ssh-deploy-root-remote remote-path)
      (message "registered ssh-deploy for this buffer to %s" ssh-deploy-root-remote)
      ;; AI remove (local . remote) from stfl/ssh-deploy-local-mappings
      ))
