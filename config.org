...
'(face indentation space-after-tab indentation::tab indentation::space tabs tab-mark spaces space-mark newline newline-mark trailing lines-tail)
         ))
#+end_src

Visually highlty if an indentation issue was discovered which emacs already does for us
